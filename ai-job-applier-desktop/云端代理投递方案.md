# 云端代理投递方案 - 技术实现

## 架构

```
学生 Web 界面 → FastAPI → Redis 队列 → Celery Worker → Playwright 投递
```

## 核心代码

### 1. 任务队列（backend/tasks.py）

```python
from celery import Celery
from playwright.async_api import async_playwright
import asyncio

# Celery 配置
celery_app = Celery(
    'job_applier',
    broker='redis://localhost:6379/0',
    backend='redis://localhost:6379/1'
)

@celery_app.task(bind=True)
def apply_job_task(self, user_id: str, job_data: dict, resume_text: str):
    """
    异步投递任务
    """
    try:
        # 更新任务状态
        self.update_state(state='PROGRESS', meta={'status': '正在投递...'})
        
        # 调用 Playwright 投递
        result = asyncio.run(do_apply(job_data, resume_text))
        
        # 返回结果
        return {
            'success': True,
            'job_title': job_data['title'],
            'company': job_data['company'],
            'result': result
        }
    except Exception as e:
        return {
            'success': False,
            'error': str(e)
        }

async def do_apply(job_data: dict, resume_text: str):
    """
    实际投递逻辑
    """
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page()
        
        # 加载 Cookie（从数据库获取）
        # await page.context.add_cookies(cookies)
        
        # 访问岗位页面
        await page.goto(job_data['url'])
        
        # 点击投递按钮
        await page.click('button:has-text("立即沟通")')
        
        # 发送打招呼消息
        await page.fill('.chat-input', '您好，我对这个岗位很感兴趣...')
        await page.click('.send-btn')
        
        await browser.close()
        return True
```

### 2. API 接口（backend/api/apply.py）

```python
from fastapi import APIRouter, Depends, HTTPException
from tasks import apply_job_task
from auth import get_current_user

router = APIRouter()

@router.post("/api/submit-apply")
async def submit_apply(
    job_data: dict,
    resume_text: str,
    user = Depends(get_current_user)
):
    """
    提交投递任务
    """
    # 检查用户额度
    if user.remaining_quota <= 0:
        raise HTTPException(400, "投递次数已用完，请升级套餐")
    
    # 提交到队列
    task = apply_job_task.delay(
        user_id=user.id,
        job_data=job_data,
        resume_text=resume_text
    )
    
    # 扣除额度
    user.remaining_quota -= 1
    user.save()
    
    return {
        "task_id": task.id,
        "status": "queued",
        "remaining_quota": user.remaining_quota
    }

@router.get("/api/task-status/{task_id}")
async def get_task_status(task_id: str):
    """
    查询任务状态
    """
    task = apply_job_task.AsyncResult(task_id)
    
    if task.state == 'PENDING':
        return {"status": "pending", "message": "排队中..."}
    elif task.state == 'PROGRESS':
        return {"status": "progress", "message": task.info.get('status')}
    elif task.state == 'SUCCESS':
        return {"status": "success", "result": task.result}
    else:
        return {"status": "failed", "error": str(task.info)}
```

### 3. 用户认证（backend/auth.py）

```python
from fastapi import Depends, HTTPException
from fastapi.security import HTTPBearer
import jwt

security = HTTPBearer()

def get_current_user(token = Depends(security)):
    """
    验证用户 Token
    """
    try:
        payload = jwt.decode(token.credentials, SECRET_KEY, algorithms=["HS256"])
        user_id = payload.get("user_id")
        
        # 从数据库获取用户
        user = db.get_user(user_id)
        if not user:
            raise HTTPException(401, "用户不存在")
        
        return user
    except:
        raise HTTPException(401, "Token 无效")
```

### 4. 前端调用（Streamlit）

```python
import streamlit as st
import requests
import time

st.title("🚀 AI 求职助手 - 自动投递")

# 用户登录
token = st.session_state.get('token')
if not token:
    st.warning("请先登录")
    st.stop()

# 上传简历
resume_file = st.file_uploader("上传简历", type=['pdf', 'docx'])

# 搜索岗位
keyword = st.text_input("搜索关键词", "Python 实习")
city = st.text_input("城市", "北京")

if st.button("搜索岗位"):
    # 调用搜索 API
    response = requests.post(
        "http://your-server.com/api/search-jobs",
        json={"keyword": keyword, "city": city},
        headers={"Authorization": f"Bearer {token}"}
    )
    jobs = response.json()['jobs']
    st.session_state['jobs'] = jobs

# 显示岗位列表
if 'jobs' in st.session_state:
    for job in st.session_state['jobs']:
        col1, col2 = st.columns([3, 1])
        with col1:
            st.write(f"**{job['title']}** - {job['company']}")
            st.write(f"💰 {job['salary']}")
        with col2:
            if st.button("投递", key=job['id']):
                # 提交投递任务
                response = requests.post(
                    "http://your-server.com/api/submit-apply",
                    json={
                        "job_data": job,
                        "resume_text": resume_text
                    },
                    headers={"Authorization": f"Bearer {token}"}
                )
                
                task_id = response.json()['task_id']
                
                # 轮询任务状态
                with st.spinner("投递中..."):
                    while True:
                        status_response = requests.get(
                            f"http://your-server.com/api/task-status/{task_id}"
                        )
                        status = status_response.json()
                        
                        if status['status'] == 'success':
                            st.success("✅ 投递成功！")
                            break
                        elif status['status'] == 'failed':
                            st.error(f"❌ 投递失败: {status['error']}")
                            break
                        
                        time.sleep(2)
```

## 部署方案

### 方案 A：你的电脑 24 小时开机

```bash
# 1. 启动 Redis
redis-server

# 2. 启动 Celery Worker
celery -A tasks worker --loglevel=info

# 3. 启动 FastAPI
uvicorn main:app --host 0.0.0.0 --port 8765

# 4. 使用内网穿透（frp/ngrok）
# 让学生可以访问你的本地服务
```

**成本**：¥0（用你的电脑）

### 方案 B：租云服务器

```bash
# 阿里云/腾讯云轻量服务器
# 配置：2核4G，5M 带宽
# 价格：¥100-200/月

# 部署步骤同上
```

**成本**：¥100-200/月

### 方案 C：Serverless（推荐）

```bash
# 使用 Railway/Render/Fly.io
# 自动扩容，按需付费
# 价格：¥50-100/月（小规模）
```

**成本**：¥50-100/月

## 定价策略

### 免费版
- 每天 5 次投递
- 体验所有功能
- 吸引用户注册

### 基础版 - ¥19.9/月
- 每天 30 次投递
- AI 生成求职信
- 投递记录管理
- **目标用户**：普通大学生

### 专业版 - ¥39.9/月
- 每天 100 次投递
- 优先投递（跳过队列）
- 简历优化建议
- 数据分析报告
- **目标用户**：急需找工作的学生

### 年费版 - ¥199/年
- 无限次投递
- 所有功能
- **目标用户**：长期找工作的学生

## 收入预测

### 第 1 个月
- 注册用户：100 人
- 付费用户：10 人（10% 转化率）
- 客单价：¥19.9
- **收入：¥199**

### 第 3 个月
- 注册用户：500 人
- 付费用户：75 人（15% 转化率）
- 客单价：¥25（混合）
- **收入：¥1,875**

### 第 6 个月
- 注册用户：2000 人
- 付费用户：400 人（20% 转化率）
- 客单价：¥30（混合）
- **收入：¥12,000**

## 推广渠道

### 1. 小红书（主战场）
- 标题："大学生必备！自动投递简历神器"
- 内容：使用教程 + 效果展示
- 目标：每周 3 篇笔记

### 2. 知乎
- 回答："有哪些大学生找实习的技巧？"
- 回答："如何提高简历投递效率？"
- 目标：每周 5 个回答

### 3. B站
- 视频："我用这个工具 3 天拿到 5 个实习 offer"
- 视频："大学生找实习必备工具测评"
- 目标：每周 1 个视频

### 4. 校园代理
- 招募校园大使
- 每推荐 1 人付费，奖励 ¥5
- 目标：10 个学校，各 1 个代理

## 技术优势

1. **无需安装** - 打开网页就能用
2. **实时反馈** - WebSocket 推送进度
3. **数据安全** - 简历不存储，用完即删
4. **持续优化** - 云端更新，学生无感知
5. **防封号** - 你控制投递频率和策略

## 风险控制

1. **Boss 账号管理**
   - 准备 5-10 个 Boss 账号
   - 轮换使用，避免单个账号被封

2. **投递频率限制**
   - 每个账号每天最多投 50 个
   - 自动分配任务到不同账号

3. **法律风险**
   - 添加免责声明
   - 用户协议明确责任

## 下一步行动

### 今晚（2 小时）
1. ✅ 安装 Redis 和 Celery
2. ✅ 实现基础任务队列
3. ✅ 测试投递流程

### 明天（4 小时）
1. ✅ 添加用户认证
2. ✅ 添加计费系统
3. ✅ 部署到云服务器

### 后天（4 小时）
1. ✅ 写第一篇小红书笔记
2. ✅ 录制使用教程视频
3. ✅ 找 10 个学生内测

### 第一周目标
- 50 个注册用户
- 5 个付费用户
- ¥100 收入
```

